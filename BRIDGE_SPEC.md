...
├── gemini-extension.json          (manifest v2.0.0 — at repo root)
├── GEMINI.md                      (context file — auto-loaded at session start)
├── BRIDGE_SPEC.md                 (this file)
├── agents/
│   └── code-reviewer.md
├── commands/                      (12 commands — 3 core + 8 shortcuts + 1 alias)
│   ├── brainstorm.toml
│   ├── write-plan.toml
│   ├── execute-plan.toml
│   ├── plan.toml
...
└── skills/                        (14 skills — self-contained within extension)
...
# NOTE: v2.0.0 — skills moved from global ~/.gemini/skills/ into extension
# NOTE: v2.0.0 — gemini-extension.json moved to repo root for `gemini extensions install`
```
...
| 2.0.0 | 2026-04-13 | Restructured for `gemini extensions install` compatibility. Flattened directory (removed extensions/ subdirectory). Skills moved from global ~/.gemini/skills/ into extension. Added GEMINI.md context file. Added 9 shortcut commands. Added openai.yaml for Codex compatibility. |
